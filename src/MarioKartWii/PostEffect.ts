...
import * as KCL from './kcl.js'; // Added KCL Parser Import
...
class MarioKartWiiRenderer {
    ...
    private setMirrored(mirror: boolean): void {
        ...
        for (let i = 0; i < this.baseObjects.length; i++) {
            ...
            // Quick check to make sure the object actually has a model instance
            if (!(this.baseObjects[i] instanceof KCLRenderer)) {
    ...
    public createPanels(): UI.Panel[] {
        ...
        // Added Show Collision checkbox
        const showCollision = new UI.Checkbox('Show Collision', false);
        showCollision.onchanged = () => {
            const v = showCollision.checked;
            for (let i = 0; i < this.baseObjects.length; i++) {
                if (this.baseObjects[i] instanceof KCLRenderer) {
                    this.baseObjects[i].visible = v;
                }
            }
        };
        renderHacksPanel.contents.appendChild(showCollision.elem);
...
// Added KCLRenderer wrapper class
class KCLRenderer implements BaseObject {
    public visible = false; // Kept off by default
    public modelMatrix = mat4.create();

    constructor(public kclRenderInst: any) {
        mat4.copy(this.modelMatrix, posMtx); // Scale it by 0.1 just like the course model
    }
    ...
    public prepareToRender(device: GfxDevice, renderInstManager: GfxRenderInstManager, viewerInput: Viewer.ViewerRenderInput): void {
        ...
        // Ensure kcl.ts handles the matrix multiplication in its prepareToRender
        this.kclRenderInst.prepareToRender(device, renderInstManager, viewerInput, this.modelMatrix);
    }

    public destroy(device: GfxDevice): void {
        if (this.kclRenderInst && this.kclRenderInst.destroy) {
            this.kclRenderInst.destroy(device);
        }
    }
}
...
class MarioKartWiiSceneDesc implements Viewer.SceneDesc {
    ...
    public static async createSceneFromU8Archive(context: SceneContext, arc: U8.U8Archive): Promise<MarioKartWiiRenderer> {
        ...
        // Added KCL Loading Hook
        const kclData = arc.findFileData(`./course.kcl`);
        if (kclData !== null) {
            const parsedKCL = KCL.parse(kclData);
            const kclMesh = new KCL.KCLRenderMesh(device, cache, parsedKCL);
            
            const kclInstance = new KCLRenderer(kclMesh);
            renderer.baseObjects.push(kclInstance);
        }
